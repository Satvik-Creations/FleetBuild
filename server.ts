...
async function startServer() {
  ...
  // POST /api/voice-coach/talk (OpenRouter Voice Coach Response using fish-audio/s2.1-pro-free:free)
  app.post('/api/voice-coach/talk', async (req, res) => {
    try {
      const { message, topic } = req.body || {};
      const openRouterKey = process.env.OPENROUTER_API_KEY || process.env.OPEN_ROUTER_API_KEY;

      const userPrompt = message || `Athlete wants advice on topic: ${topic || 'Form & Movement Check'}`;
        ...
        try {
          const response = await fetch('https://openrouter.ai/api/v1/chat/completions', {
            ...
            headers: {
              ...
              'X-Title': 'FleetBuild Voice Coach',
            },
            body: JSON.stringify({
          ...
          if (response.ok) {
            ...
            if (replyText) {
              return res.json({ reply: replyText, model: 'fish-audio/s2.1-pro-free:free' });
            }
          }
        } catch (openRouterErr) {
          console.warn('OpenRouter API call failed, falling back to default voice coach responses:', openRouterErr);
        }
      }

      // Fallback voice coach responses for deep male trainer voice
      const fallbackReplies: Record<string, string[]> = {
        'Form & Movement Check': [
          "FleetBot Coach here! Lock your shoulder blades down and back. Squeeze your core tight before every rep!",
          "Keep your posture upright and drive through your heels. Control the weight on the way down!",
          "Focus on full range of motion. Don't rush the reps—make every single second count!",
        ],
        'Warm-Up & Mobility': [
          "Get those joints lubricated! Start with arm circles, hips mobility, and 10 dynamic bodyweight squats.",
          "Prioritize shoulder and ankle mobility before heavy lifting. A solid warm-up prevents injuries and boosts power output!",
        ],
        'Post-Workout Cool Down': [
          "Great effort out there! Take deep diaphragmatic breaths and hold your static stretches for 30 seconds each.",
          "Hydrate immediately and refuel with high quality protein. Your recovery starts right now brother!",
        ],
        'Goal & Mindset Boost': [
          "Remember why you started! Discipline beats motivation every single day. Bring that fire to your next set!",
          "You are stronger than you think. Block out the fatigue, focus on the weight, and conquer this workout!",
        ],
      };

      const topicGroup = fallbackReplies[topic] || fallbackReplies['Form & Movement Check'];
      const randomReply = topicGroup[Math.floor(Math.random() * topicGroup.length)];

      res.json({
        reply: randomReply,
        model: 'fish-audio/s2.1-pro-free:free (simulated fallback)',
      });
    } catch (err) {
      console.error('Error in voice coach endpoint:', err);
      res.status(500).json({ error: 'Failed to generate voice coach response.' });
    }
  });
